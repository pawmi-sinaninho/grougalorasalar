...
- **Rejet:** target an any-colour pillar on one of the eight cardinal/diagonal rays at one or two aligned logical steps. The destination is a fixed radius from the targeted pillar: three cardinal cells or two diagonal cells. Therefore an adjacent cardinal target moves the player two cells, and an adjacent diagonal target moves the player one diagonal cell. Every traversed cell and the destination must be free and inside the arena. A pillar, obstacle or arena edge anywhere on that movement path makes the cast illegal; movement is never truncated. A zero-displacement range-two diagonal cast is legal but does not satisfy the mandatory movement needed to end the round.
