...
def _actions_for(
    ...
    definite, conditional = solver._enumerate_actions(
        ...
        spell_values={spell: None for spell in SPELLS},
        budget=12,
...
def test_rejet_targets_any_pillar_on_eight_rays_up_to_two_cells() -> None:
    ...
    actions = _actions_for("repulsion", pillars)
...
def test_attrait_targets_any_linear_pillar_from_one_to_six_cells() -> None:
    for distance in range(1, 7):
        actions = _actions_for(
            "attraction",
            [_pillar(f"P{distance}", (distance, 0), "reflection")],
        )

        assert len(actions) == 1
        assert _target_cells(actions) == {(distance, 0)}
        assert actions[0]["targetKind"] == "pillar"
        assert _destination(actions[0]) == (min(distance - 1, 3), 0)
