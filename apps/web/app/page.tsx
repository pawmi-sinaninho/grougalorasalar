...
<circle cx={pin.x} cy={pin.y} r="22" className="action-pin" />
          ...
          <div className="analysis-strip" aria-live="polite"><strong>{busy ? (data ? 'Analyse de la capture… (15–20 s)' : 'Chargement de l’arène… (30–35 s)') : recommendation ? 'Solution prête' : stageLabels[progress.stage] ?? 'Analyse en cours'}</strong>{debug && <span>{Math.round(elapsedMs || progress.elapsedMs)} ms</span>}</div>
